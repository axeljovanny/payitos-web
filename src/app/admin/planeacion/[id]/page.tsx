...
import { approveWeeklyPlan, updateWeeklyPlan, deleteWeeklyPlan, reopenWeeklyPlan } from '@/lib/planeacion/actions'
...
export default async function AdminPlaneacionDetailPage({ params }: Props) {
    ...
    <div className="space-y-5">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <Link href={BASE} className="text-xs mb-1 inline-block" style={{ color: 'var(--ink-faint)' }}>
            ← Planeación
          </Link>
        ...
        <div className="flex gap-2 shrink-0">
          ...
          <form action={deleteWeeklyPlan}>
            ...
            <button
              type="submit"
              className="rounded-xl text-sm font-bold px-3.5 py-2.5 transition-colors active:scale-[0.97]"
      ...
      {!isDraft && (
        <div className="space-y-2">
          ...
          <form action={reopenWeeklyPlan}>
            <input type="hidden" name="plan_id" value={plan.id} />
            <input type="hidden" name="return_path" value={`${BASE}/${plan.id}/editar`} />
            <button
              type="submit"
              className="w-full rounded-2xl text-sm font-bold px-4 py-3.5 transition-[transform,box-shadow] active:scale-[0.98]"
              style={{
